...
//!
//! `Event::PtyWrite` is forwarded so CSI responses (e.g. DSR cursor position)
//! are written back to the PTY — required for PowerShell and similar shells.
//!
//! # Example
//!
//! ```
...
/// consumed by the GPUI application on the main thread. `PtyWrite` is written
/// synchronously to the PTY so shells waiting on CSI replies do not stall.
pub struct GpuiEventProxy {
    /// Channel sender for forwarding events to the GPUI application.
    tx: Sender<TerminalEvent>,
    /// PTY stdin for immediate `PtyWrite` writeback.
    stdin: Arc<Mutex<Box<dyn Write + Send>>>,
}

impl GpuiEventProxy {
    /// Creates a new event proxy with the given channel sender and PTY writer.
    pub fn new(tx: Sender<TerminalEvent>, stdin: Arc<Mutex<Box<dyn Write + Send>>>) -> Self {
        Self { tx, stdin }
    }
...
mod tests {
    ...
    fn proxy_with(tx: Sender<TerminalEvent>) -> GpuiEventProxy {
        let stdin = Arc::new(Mutex::new(
            Box::new(Cursor::new(Vec::new())) as Box<dyn Write + Send>
        ));
        GpuiEventProxy::new(tx, stdin)
    }

    #[test]
    fn test_event_proxy_creation() {
        let (tx, _rx) = channel();
        let _proxy = proxy_with(tx);
    }

    #[test]
    fn test_wakeup_event() {
        let (tx, rx) = channel();
        let proxy = proxy_with(tx);
    ...
    #[test]
    fn test_pty_write_goes_to_stdin() {
        ...
        let stdin = Arc::new(Mutex::new(
            Box::new(Shared(store.clone())) as Box<dyn Write + Send>
        ));
        let proxy = GpuiEventProxy::new(tx, stdin);

        proxy.send_event(Event::PtyWrite("\x1b[1;1R".into()));

        assert_eq!(store.lock().as_slice(), b"\x1b[1;1R");
