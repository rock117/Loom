...
| 侧栏 | 对应 Profile | 无 |
| 重启 | 可进 `open_tabs` | 不恢复 |
| 凭据 | `profile_id` | `auth_profile_id`（从源会话继承） |

**凭据查找**一律用 `credentials_profile_id() = profile_id.or(auth_profile_id)`（密码 / keyring / 转发 / 重连弹窗）。  
...
10. **SSH 密码 UI**（打开 / Duplicate / Split / Reconnect）与底层 resolve 同一套 `credentials_profile_id`；缺密码弹同一 PasswordPrompt，禁止只改 status / `eprintln`。
11. **Reconnect 粒度**：状态栏 = 该 Tab **全部** Pane；Failed 占位按钮 = **单** Pane；两者密码路径对称。
...
| 来源 | `profile_id` | `auth_profile_id` | 重启 |
|------|--------------|-------------------|------|
| 侧栏打开 | `Some` | `None` | 可恢复 |
| Ctrl+T | `None` | `None` | 否 |
| Duplicate Tab / Split（自 Bound 或带凭据的 Ephemeral） | `None` | 继承 credentials | 否 |
| Save to… | 变为 `Some` | 清掉 | 之后可恢复 |
...
| Tab 栏 / 终端菜单 / Ctrl+Shift+D | 再开 **临时** Tab（克隆 focused Pane） |

## 持久化

- `workspace.json`：根 `profiles` + 嵌套 `groups`。  
- `ui_state.open_tabs`：仅 Bound Tab（`bound_profile_id()`）。  
- 启动：只按 Bound 重开（单 Pane；split 树暂不持久）。

## 实现锚点

- `PaneSession` / `TabSession` / `SessionOpResult`：`src/ui/tab_manager.rs`
